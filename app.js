(() => {
  ...
  const ACTION_PERMISSION = Object.freeze({
    ...
    recovery:'AUDIT_OPERATIONAL', opening:'OWNER_ONLY', financeActual:'FINANCE_REPORT', stockReport:'FINANCE_REPORT', materials:'ANY', hppCorrection:'OWNER_ONLY'
  });
  ...
  function renderControl() {
    ...
    const all = [
      ...
      ['money','Posisi Keuangan Aktual','Rekonsiliasi Bank & Kas tanpa mengubah ledger','financeActual'],
      ['clipboard','Laporan Stok Warehouse','Mingguan / bulanan · pembelian & distribusi outlet','stockReport'],
      ['wallet','Saldo Awal','Baseline Bank / Kas Gudang','opening'],
  ...
  function directOpening(){
    const c=(state.data||{}).controls||{},bankRecorded=Number(c.openingBankRecorded||0),cashRecorded=Number(c.openingCashRecorded||0),bankCurrent=Number(c.currentBankBalance||0),cashCurrent=Number(c.currentCashBalance||0),pending=!!(c.openingBankReentryPending||c.openingCashReentryPending);
    sheetHtml('Saldo Awal',`<div class="role-guide"><b>Baseline saja</b><span>Menu ini khusus menetapkan saldo awal Bank dan Kas Gudang. Untuk mencatat kondisi rekening/kas yang nyata saat ini gunakan <b>Posisi Keuangan Aktual</b>.</span><span>Perubahan saldo awal tetap memakai transaksi koreksi delta dan audit trail; ledger lama tidak dihapus.</span></div>${pending?'<div class="demo-box"><b>Perlu input ulang.</b><br>Canonical v1.3.0 mengembalikan komponen saldo awal ke Rp0 sesuai instruksi Owner. Isi kembali baseline Bank/Kas yang benar sebelum mengandalkan saldo sistem untuk transaksi keuangan.</div>':''}<div class="stats-mini"><div class="demo-box"><b>Bank</b><br>Saldo awal tercatat ${rp(bankRecorded)}<br>Saldo sistem sekarang ${rp(bankCurrent)}</div><div class="demo-box"><b>Kas Gudang</b><br>Saldo awal tercatat ${rp(cashRecorded)}<br>Saldo sistem sekarang ${rp(cashCurrent)}</div></div><form id="ghOpenBank"><label class="field"><span>Nama rekening</span><input name="accountName" value="Bank Warehouse" required></label><label class="field"><span>Target Saldo Awal Bank</span><input name="amount" type="number" min="0" step="1" value="${bankRecorded}" required></label><label class="field"><span>Alasan / catatan perubahan</span><textarea name="note" placeholder="Contoh: baseline Bank yang benar"></textarea></label><button class="btn btn-primary">Simpan Saldo Awal Bank</button></form><form id="ghOpenCash"><label class="field"><span>Referensi</span><input name="reference" value="Kas Gudang"></label><label class="field"><span>Target Saldo Awal Kas</span><input name="amount" type="number" min="0" step="1" value="${cashRecorded}" required></label><label class="field"><span>Alasan / catatan perubahan</span><textarea name="note" placeholder="Contoh: baseline Kas Gudang yang benar"></textarea></label><button class="btn btn-primary">Simpan Saldo Awal Kas</button></form>`);
    $('#ghOpenBank').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target);await writeDirect('opening_bank','setOpeningBankBalance',{accountName:fd.get('accountName'),amount:fd.get('amount'),note:fd.get('note')})};
    $('#ghOpenCash').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target);await writeDirect('opening_cash','setOpeningCashBalance',{reference:fd.get('reference'),amount:fd.get('amount'),note:fd.get('note')})};
  }
  async function directFinanceActualV1300(){
    let r;await withBusy('Membaca posisi keuangan…',async()=>{r=await callDirect('getFinancialPositionActualV1300')});const a=r.actual||null,d=r.currentDifference||null,stale=r.actualStale,canWrite=!!r.canWrite;const badge=(v)=>Math.abs(Number(v||0))<0.5?'<span class="badge ok">Rp0 · SESUAI</span>':`<span class="badge bad">Selisih ${rp(v)}</span>`;
    sheetHtml('Posisi Keuangan Aktual',`<div class="role-guide"><b>Rekonsiliasi, bukan edit ledger</b><span>Saldo Sistem tetap berasal dari transaksi Warehouse. Saldo Aktual adalah hasil cek rekening dan uang fisik oleh Owner.</span><span>Menyimpan posisi aktual tidak mengubah Saldo Awal, Ledger Bank, Ledger Kas, stok, piutang, atau hutang.</span></div>${stale?'<div class="demo-box"><b>Aktual perlu diperbarui.</b><br>Ada transaksi Bank/Kas setelah pengecekan aktual terakhir.</div>':''}<div class="stats-mini"><div class="demo-box"><b>Bank</b><br>Sistem ${rp(r.system.bank)}<br>Aktual ${a?rp(a.bankActual):'Belum diinput'}<br>${a?badge(d.bank):''}</div><div class="demo-box"><b>Kas Gudang</b><br>Sistem ${rp(r.system.cash)}<br>Aktual ${a?rp(a.cashActual):'Belum diinput'}<br>${a?badge(d.cash):''}</div></div>${a?`<div class="demo-box">Terakhir dicek ${esc(a.time)} oleh ${esc(a.user)}${a.note?'<br>'+esc(a.note):''}</div>`:''}${canWrite?`<form id="ghFinanceActual"><label class="field"><span>Saldo Aktual Bank</span><input name="bankActual" type="number" min="0" step="1" value="${a?Number(a.bankActual):0}" required></label><label class="field"><span>Kas Aktual Gudang</span><input name="cashActual" type="number" min="0" step="1" value="${a?Number(a.cashActual):0}" required></label><label class="field"><span>Catatan rekonsiliasi</span><textarea name="note" placeholder="Contoh: cek mobile banking dan hitung kas fisik"></textarea></label><button class="btn btn-primary">Simpan Posisi Aktual</button></form>`:'<div class="demo-box">Mode baca saja. Hanya Owner yang dapat menyimpan posisi aktual.</div>'}`);
    const f=$('#ghFinanceActual');if(f)f.onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target),idem=idemKey('finance_actual');beginBusy('Menyimpan posisi aktual…');try{const x=await bridge.call('saveFinancialPositionActualV1300',state.token,{bankActual:fd.get('bankActual'),cashActual:fd.get('cashActual'),note:fd.get('note')},idem);localStorage.removeItem('ww_gh_idem_finance_actual');toast('Posisi aktual tersimpan.');closeSheet();refreshBootstrapLight().catch(()=>{});}catch(err){toast(err.message)}finally{endBusy()}};
  }
  let lastStockReportV1300=null;
  function todayIsoV1300(){const d=new Date(),p=n=>String(n).padStart(2,'0');return `${d.getFullYear()}-${p(d.getMonth()+1)}-${p(d.getDate())}`}
  async function directStockReportV1300(){
    sheetHtml('Laporan Stok Warehouse',`<div class="role-guide"><b>Laporan dari ledger canonical</b><span>Pembelian dihitung dari event PURCHASE. Barang keluar outlet dihitung dari DISTRIBUTION_DISPATCH dan dipisah per outlet melalui Surat Jalan.</span><span>Mode Mingguan = Senin–Minggu. Mode Bulanan = tanggal 1 sampai akhir bulan.</span></div><form id="ghStockReport"><div class="form-2"><label class="field"><span>Periode</span><select name="mode"><option value="WEEK">Mingguan</option><option value="MONTH">Bulanan</option></select></label><label class="field"><span>Tanggal acuan</span><input type="date" name="anchorDate" value="${todayIsoV1300()}" required></label></div><button class="btn btn-primary">Buat Laporan</button></form><div id="ghStockReportResult"></div>`);
    $('#ghStockReport').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target),box=$('#ghStockReportResult');let r;await withBusy('Menghitung laporan stok…',async()=>{r=await callDirect('getWarehouseStockMovementReportV1300',{mode:fd.get('mode'),anchorDate:fd.get('anchorDate')})});lastStockReportV1300=r;const rows=r.rows||[];box.innerHTML=`<div class="demo-box"><b>${esc(r.period.startDate)} s/d ${esc(r.period.endDate)}</b><br>Pembelian ${rp(r.summary.purchaseValue)} · Keluar outlet (HPP) ${rp(r.summary.outboundHppValue)}<br>Nilai stok awal ${rp(r.summary.openingStockValue)} → akhir ${rp(r.summary.closingStockValue)}${r.unmappedDispatchCount?'<br><b>Perhatian:</b> '+num(r.unmappedDispatchCount)+' dispatch belum terpetakan outlet.':''}</div><div class="direct-list">${rows.map(x=>`<div class="direct-card"><b>${esc(x.name)}</b><small>${esc(x.code)} · Stok awal ${num(x.openingQtyUnit)} ${esc(x.receiveUnit)} → akhir ${num(x.closingQtyUnit)} ${esc(x.receiveUnit)}</small><small>Dibeli <b>${num(x.purchaseQtyUnit)} ${esc(x.receiveUnit)}</b> (${rp(x.purchaseValue)}) · Keluar outlet <b>${num(x.outletQtyUnit)} ${esc(x.receiveUnit)}</b> (${rp(x.outletHppValue)} HPP)</small>${(x.outlets||[]).length?`<small>${x.outlets.map(o=>`${esc(o.outletName)}: ${num(o.qtyUnit)} ${esc(x.receiveUnit)}`).join(' · ')}</small>`:''}</div>`).join('')||'<div class="empty">Tidak ada pergerakan stok pada periode ini.</div>'}</div>${(r.issues||[]).length?`<div class="demo-box"><b>Audit warning</b><br>${r.issues.map(esc).join('<br>')}</div>`:''}<div class="actions"><button class="btn btn-soft" id="ghPrintStockReport">Cetak / Simpan PDF</button></div>`;$('#ghPrintStockReport').onclick=()=>printStockReportV1300(r);};
  }
