(() => {
  ...
  // v1.2.7.3 — consolidated auto-price + Owner controls + HPP foundation.
  ...
  const ROLE_PERMISSIONS = Object.freeze({
    ...
    ADMIN_1:['PURCHASE','PACKING','EXPENSE','SUPPLIER_MASTER','INTERNAL_PRICE','OUTLET_PAYMENT','SUPPLIER_PAYMENT','PACKING_WAGE_PAYMENT','AUDIT_OPERATIONAL'],
  ...
  function renderControl() {
    const d = state.data || {};
    const all = [
      ['price','Harga Internal Outlet','Otomatis mengikuti harga Terima Bahan Outlet','internalPrice'],
  ...
  function directInternalPrice(){
    sheetHtml('Harga Internal Outlet',`<div class="role-guide"><b>Otomatis dari Terima Bahan Outlet</b><span>Harga internal tidak perlu diinput ulang. Warehouse membaca harga terbaru per bahan dari Terima Bahan dan hanya mengubah harga jika data antar-outlet konsisten.</span><span>Snapshot Surat Jalan lama tetap terkunci; perubahan hanya berlaku untuk SJ baru.</span></div><div class="actions"><button class="btn btn-primary" type="button" id="ghSyncPrice">Sinkron Harga Sekarang</button></div><div id="ghPricePreview"><div class="demo-box">Membaca harga Terima Bahan…</div></div>`);
    const render=async()=>{try{let r;await withBusy('Membaca harga Terima Bahan…',async()=>{r=await callDirect('previewOutletReceiptInternalPrices')});const st=r.stats||{},rows=(r.rows||[]).filter(x=>x.status!=='SAME'),errors=r.errors||[];$('#ghPricePreview').innerHTML=`<div class="demo-box">Perlu update ${st.ready||0} · Sudah sama ${st.same||0} · Konflik ${st.conflict||0} · Belum ada ${st.missing||0}</div>${errors.length?`<div class="demo-box">Auto-sync ditahan: ${errors.map(x=>`${esc(x.outletName||x.outletCode)} — ${esc(x.error)}`).join('<br>')}</div>`:''}<div class="direct-list">${rows.map(x=>`<div class="direct-card"><b>${esc(x.name)}</b><small>Warehouse ${Number(x.currentPrice||0)>0?rp(x.currentPrice):'BELUM DISET'} · ${x.status==='READY'?`Terima Bahan ${rp(x.suggestedPrice)} · akan otomatis`:x.status==='CONFLICT'?'KONFLIK antar-outlet':x.status==='UNIT_MISMATCH'?'SATUAN BERBEDA':'BELUM ADA DATA'}</small></div>`).join('')||'<div class="empty">Semua harga yang tersedia sudah sama.</div>'}</div>`}catch(e){$('#ghPricePreview').innerHTML=`<div class="demo-box">${esc(e.message)}</div>`;toast(e.message)}};
    $('#ghSyncPrice').onclick=async()=>{await writeDirect('internalPriceAuto','syncOutletReceiptInternalPricesNow',{})};
    render();
  }
  ...
  if ('serviceWorker' in navigator) {
    ...
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=0.4.3', { updateViaCache: 'none' }).then(reg => reg.update()).catch(() => {}));
  }
